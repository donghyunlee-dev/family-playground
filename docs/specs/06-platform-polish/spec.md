# Phase 06 Spec

## Scope

This phase covers final quality improvements before considering the MVP complete.

## Requirements

- improve UI polish across core routes
- use a family-friendly visual direction suited to parents and elementary-school children
- prefer colorful sections, clear spacing, and easy-to-scan copy over dense dashboard layouts
- use Korean across the main user-facing routes unless a technical identifier must remain as-is
- ensure buttons and interactive states preserve readable contrast in hover and active states
- resolve bugs from earlier phases
- improve performance where needed
- ensure documentation reflects the implemented platform
- keep a maintained UI regression checklist and use it after every UI-affecting change

## Completion Standard

- earlier phase acceptance criteria remain valid
- regression verification is documented
